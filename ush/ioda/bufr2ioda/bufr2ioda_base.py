...
class Bufr2IodaBase:
    ...
    @timing_decorator
    def execute(self):
        data = bufr.parse(json.dumps(self.yaml_config), 0)
        if self.splits:
            self.make_split_files(data.allSubCategories())
        self.re_map_variable(data)
        bufr.encode_save(json.dumps(self.yaml_config), data)
